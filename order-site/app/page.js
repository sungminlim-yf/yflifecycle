...
export default function Page() {
  ...
  const [editLang, setEditLang] = useState('en'); // 선호 이메일 언어 인라인 편집
  ...
  function startEdit() {
    ...
    setEditLang(shop?.preferred_language || lang);
    setContactMsg(null);
    setEditing(true);
  }
  async function saveContact() {
    ...
    try {
      const r = await fetch('/api/update-contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, contact: editContact.trim(), phone: editPhone.trim(), language: editLang }),
      });
      const data = await r.json();
      if (r.ok && data && data.ok) {
        setShop((s) => ({ ...s, contact: editContact.trim(), phone: editPhone.trim(), preferred_language: editLang }));
        setContactMsg('saved');
        setEditing(false);
      } else {
        setContactMsg('err');
              ...
              {editing ? (
                <>
                  ...
                  <select value={editLang} onChange={(e) => setEditLang(e.target.value)}>
                    {Object.keys(STRINGS).map((l) => (
                      <option key={l} value={l}>{STRINGS[l].langName}</option>
                    ))}
                  </select>
