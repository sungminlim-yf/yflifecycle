...
**현재 phase = 운영 액션 집행 중 + 온보딩 모델 v2 개정 (2026-05-29 오후)**. **n8n 워크플로우 #1 skeleton 빌드 완료** (id `twZw1wv3Fc1iJdeO`, inactive, pre-onboarding 자동 hold 로직 추가). **온보딩 모델 v2 개정**: 매직 토큰 = HubSpot Company 생성 시점에 #0이 발급, 승인 주체 = admin team, 트리거 위치 = Airtable. 자세한 진행상황은 아래 "운영 액션 진행 현황" 섹션.
