---
name: mcp-setup
description: "yflifecycle 프로젝트에 설치한 MCP 서버 구성 — 설치 방식, 패키지, 주의점 (6개 전부 설치 완료)"
metadata: 
  node_type: memory
  type: project
...
- **Xero** ✅ — `@xeroapi/xero-mcp-server@latest` (공식). Custom Connection `claude-yflifecycle-v1` 사용. env: `XERO_CLIENT_ID`, `XERO_CLIENT_SECRET` (client credentials grant, single-tenant). 도구 prefix `mcp__xero__*` — invoice/contact/item/payment/bank-transaction/credit-note/quote/manual-journal/timesheet + 회계 리포트(p&l, balance sheet, trial balance, aged payables/receivables) + accounts/tax-rates/tracking-categories. 키 출처는 [[xero-account]].
